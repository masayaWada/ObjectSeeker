...
- **🔍 オブジェクト検索**: ユーザー、グループ、アプリケーションを名前・メール等で検索してオブジェクトIDを取得
- **🔁 プリンシパルID逆引き**: オブジェクトID（GUID）から種別（user / group / servicePrincipal）と詳細情報を完全一致で逆引き
- **🛡️ ロール検索**: サブスクリプション・リソースグループを選択（サジェスト機能付き）してロール割り当て／ロール定義を検索
- **📦 一括検索**: グループ名・アプリケーション名を複数まとめて入力し、存在の有無を一括判定（完全一致=〇 / 部分一致=△ / 存在なし=✕）
- **🎯 タブ型GUI**: tkinterベースの「オブジェクト検索」「ロール検索」「一括検索」タブ構成
- **📋 列単位コピー**: 検索結果のセルをダブルクリックすると、その列の値（名前・メール・オブジェクトID・ロール名など）がクリップボードへコピーされる
- **🔐 Azure CLI認証**: `az login`を使用した簡単な認証（本アプリはトークンを保持しない）
- **⚙️ 設定可能**: JSON設定ファイルによるカスタマイズ
- **📝 ログ機能**: 詳細なログ出力とデバッグ情報
- **🧪 テスト対応**: `unittest.mock` ベースの包括的テストスイート
...
### 4. オブジェクト検索（「オブジェクト検索」タブ）

1. **検索タイプ**を選択：
   - `user`: ユーザー検索
   - `group`: グループ検索
   - `application`: アプリケーション検索
   - `principal_id`: プリンシパルID（GUID）からの逆引き（完全一致）

2. **検索クエリ**に入力：
   - ユーザー: メールアドレス、ユーザー名、表示名の一部
   - グループ: グループ名、メールアドレスの一部
   - アプリケーション: アプリケーション名の一部
   - プリンシパルID: オブジェクトID（GUID形式、完全一致）

3. **検索**ボタンをクリックまたはEnterキーを押下

### 5. ロール検索（「ロール検索」タブ）

1. **サブスクリプション**を選択（入力欄にサジェストが表示される）
2. **リソースグループ**を選択（省略時はサブスクリプション全体が対象）
3. **ロール名**（英語）を入力して**検索**を実行

### 6. 一括検索（「一括検索」タブ）

グループ名・アプリケーション名を1つずつ検索する手間を省き、複数まとめて存在の有無を確認できます。

1. **検索名**の入力欄に、グループ名・アプリケーション名を**1行に1つずつ**入力する
2. **チェック**ボタンを押下する
3. 結果欄に、入力した名前ごとに「**名前　判定**」の形式（名前と判定の間は全角スペース）で表示される

判定は以下のルールで自動的に行われます。

- まず**グループ**として検索し、ヒットすればグループとして判定。ヒットしなければ**アプリケーション**として検索する
- どちらかで**完全一致**すれば **〇**（種別を併記）、**部分一致**のみなら **△**（種別を併記）
- グループ・アプリケーションのいずれにもヒットしなければ **✕**（存在なし）

### 7. 結果の確認とコピー（列単位）

検索結果テーブルのセルを**ダブルクリック**すると、**クリックした列の値**がクリップボードへコピーされます。

- オブジェクト検索タブ: 名前 / 表示名 / メール / オブジェクトID / タイプ
- ロール検索タブ: ロール名（英語） / 表示名（日本語） / 説明

コピー時はステータスバーおよび確認ダイアログに「どの列をコピーしたか」が表示されます。ヘッダ行・空行・空セルをダブルクリックしても何も起きません。
...
│   ├── app.py             # メインアプリケーション（オーケストレータ）
│   ├── config.py          # 設定管理
│   ├── logger.py          # ログ管理
│   ├── exceptions.py      # 例外階層
│   ├── azure_client.py    # Azure CLI クライアント（トークン取得）
│   ├── graph_api.py       # Microsoft Graph API 検索 / プリンシパルID逆引き
│   ├── role_searcher.py   # ARM REST API ロール検索
│   ├── ui_components.py   # UIコンポーネント（AuthFrame / TabbedSearchFrame / ResultsFrame / StatusBar）
│   └── ui_theme.py        # テーマ / アクセシビリティ管理
├── tests/                 # テストコード
